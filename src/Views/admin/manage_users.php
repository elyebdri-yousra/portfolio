<main class="container mx-auto px-6 py-10 flex flex-col">
    ...
    <div class="h-16 w-full mb-6 space-x-4 flex items-center">
        <button id="page1" class="h-12 p-2 font-[Cantarell] font-semibold rounded-md text-white bg-[#DB9ECF] hover:bg-white hover:text-[#DB9ECF] duration-200" onclick="changePage(false)">Utilisateurs en attente</button>
        <button id="page2" class="h-12 p-2 font-[Cantarell] font-semibold rounded-md text-white bg-[#DB9ECF] hover:bg-white hover:text-[#DB9ECF] duration-200" onclick="changePage(true)">Liste des utilisateurs</button>
    </div>
            ...
            <tbody>
                <?php foreach ($users as $user) : ?>
                    <tr id="<?php echo $user['email'] ?>" class="border-b hover:bg-gray-50 transition duration-200">
                        ...
                        <td class="p-4 w-1/6"><input  value="Mot de passe" disabled type="password"></td>
                    ...
                    <tr id="<?php echo $user['email'] ?>" class="border-b hover:bg-gray-50 transition duration-200 editzone">
                        <form method="POST" action="index.php?page=updateUserInfo">
                            ...
                            <td class="p-4 w-1/6">
                                <select name="new_role" class="w-full h-10 indent-1 border rounded-md focus:ring focus:ring-blue-300 w-full">
                                    <?php foreach ($roles as $role) { ?>
                            ...
                            <td class="p-4 w-1/6"><input name="password" class="w-full h-10 rounded-md indent-1 bg-[#F7F5EE] shadow-inner" value="" type="password"></td>
                            <td class="p-4 w-1/6 text-center align-middle">
                                <div class="flex justify-center gap-2">
                                    <button class="p-2 bg-green-100 rounded hover:bg-green-200 transition valid-button" aria-label="Modifier l'utilisateur">
                                        <svg width="36" height="36" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z" fill="#DB9ECF" />
                                        </svg>
                                    </button>
                                    <button type="reset" class="p-2 bg-red-100 rounded hover:bg-red-200 transition reset-button" aria-label="Modifier l'utilisateur">
                                        <svg width="36" height="36" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
