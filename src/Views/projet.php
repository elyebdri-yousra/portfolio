            ...
            <?php foreach ($projets as $projet) : ?>
                <li class="border rounded-lg shadow flex flex-col gap-4 p-4 hover:shadow-md transition-shadow bg-white">
                    <?php if (!empty($projet['urlimg'])): ?>
                        <div class="relative">
                            <img src="<?php echo html_entity_decode($projet['urlimg']); ?>" alt="<?php echo html_entity_decode($projet['titre']); ?>" class="rounded w-full h-48 object-cover">
                            <a href="index.php?page=projet_show&id=<?php echo $projet['id'] ?>" class="font-[Cantarell] hover:text-[#DB9ECF] duration-200 absolute inset-0 flex items-center justify-center text-2xl font-bold text-white bg-black bg-opacity-60"><?php echo html_entity_decode($projet['titre']); ?></a>
                        </div>
                    <?php endif; ?>
                    <?php if (isset($_SESSION['user']) && ($_SESSION['user']['idRole'] == 1)) { ?>
                        <div class="flex flex-row justify-center">
                            <a href="index.php?page=supprimer&id=<?php echo $projet['id']; ?>" class="mt-auto text-red-500 hover:text-red-700 font-semibold font-[Cantarell]">Supprimer le projet</a>
                        </div>
                    <?php } ?>
                </li>
            <?php endforeach; ?>
...
<script>
    ...
    // Fermeture overlay au clic à l'extérieur du formulaire
...
<style>
    /* CSS perso à ajouter dans ton fichier app.css ou resources/css/app.css */
    .ellipsis-multiline {
        display: -webkit-box;
        -webkit-box-orient: vertical;
        overflow: hidden;
        -webkit-line-clamp: 5;
        /* ajuste selon ce que h-28 permet */
        line-clamp: 5;
    }
</style>